...
function RegisterPage() {
    ...
    async function onSubmit(e) {
        ...
        setMsg(null);
        try {
            const data = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$api$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["postJson"])("/api/auth/register", {
                email,
                password
            });
            router.replace("/login");
            setMsg("Registered ".concat(data.email));
        } catch (err) {
            const msg = String(err.message || "");
            setMsg(msg.includes("409") ? "Email already registered" : msg || "Registration failed");
        } finally{
            setLoading(false);
        }
    }
...
_s(RegisterPage, "XhYbQq/wNB3zbFGMgvdOHs54j6Y=", false, function() {
    return [
